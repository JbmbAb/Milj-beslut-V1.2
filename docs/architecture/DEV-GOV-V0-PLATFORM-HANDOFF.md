...
- **Ordinary, non-DEV-GOV pull requests** (a P1/P2/P3 feature that is not itself proving a DEV-GOV
  ...
  design audit — a candidate promoter workflow and a disabled candidate ruleset now exist
  (branch `codex/ordinary-pr-merge-path-restoration`), but nothing is live: classic branch
  protection is unchanged and the candidate ruleset's `enforcement` remains `"disabled"`.
