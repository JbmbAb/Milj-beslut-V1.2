...
export const DOCUMENT_EVIDENCE_PROPERTY_BINDING_V3_ARTIFACT_ID_PREFIX =
  'document-evidence-property-binding-v3-' as const;
...
export function createDocumentEvidencePropertyBindingArtifactV3(payload: DocumentEvidencePropertyBindingArtifactV3['payload']): DocumentEvidencePropertyBindingArtifactV3 {
  ...
  if (!Array.isArray(payload.verified_fact_refs) || payload.verified_fact_refs.length === 0) throw new Error('REJECT_DOCUMENT_EVIDENCE_PROPERTY_BINDING_V3: verified_fact_refs are required');
