...
describe('TV-4.2: QGIS Integration Architecture', () => {
  ...
  it('should extract Verified Observation as GeoJSON from CAS, strictly separating Observation from Authority', async () => {
    ...
    // The canonical presentation contract now forbids `crs` at the TYPE level too, so this cast is
    // what keeps the runtime assertion compiling -- the invariant is still checked at runtime in
    // case a projection ever reintroduces the member dynamically.
