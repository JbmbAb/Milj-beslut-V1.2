...
describe("MINIMUM-AUTHORITY-DELTA-04D — LU source authority wiring", () => {
  ...
  it("binds verified root -> issuer -> ExecutionIdentity to exactly one AuthorityEvidence on the real canonical LU assessment write", async () => {
    const f = await fixture();
    const result = await run(f);

    expect(result.admitted).toBe(true);
    expect(result.assessment).not.toBeNull();
    expect(result.assessment?.payload.assessment_contract_version).toBe(
      "localization-assessment-v4",
    );
  ...
  it("denies a correctly signed identity for a caller-chosen actor before the assessment mutation", async () => {
    const f = await fixture({
      actor_ref: { artifact_id: "attacker-chosen-actor", artifact_type: "execution_identity" },
    });
    await expect(run(f)).rejects.toThrow(
      "execution identity actor is not the canonical LU principal",
    );
    expect(
      f.repository.writes.some((write) => write.body && (write.body as { artifact_type?: string }).artifact_type === "LOCALIZATION_ASSESSMENT"),
    ).toBe(false);
  });
  ...
  it("denies a different deterministic seed and produces no authority-backed assessment", async () => {
    ...
    expect(
      f.repository.writes.some((write) => write.body && (write.body as { artifact_type?: string }).artifact_type === "authority_evidence"),
    ).toBe(false);
  });
