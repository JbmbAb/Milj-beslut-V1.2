  ...
  return {
    ...
    payload: {
      result_semantics: {
        ...
        result: {
          exists: overrides.geometry !== null,
          match_count_observed: overrides.geometry !== null ? 1 : 0,
...
describe('ViewerKernel.exportAsGeoJSON CRS transport (LU-CESIUM-GEOJSON-CRS-COMPATIBILITY-01)', () => {
  ...
  it('all geometry:null -> no unsupported crs declaration, features keep geometry:null', async () => {
    ...
    // The canonical presentation contract forbids `crs` at the type level now; the cast keeps this
    // runtime assertion compiling so the invariant stays checked in both dimensions.
    expect((geojson as unknown as Record<string, unknown>).crs).toBeUndefined();
