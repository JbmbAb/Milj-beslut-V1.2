...
router.get('/api/v1/projects', requireMvpAuth, mvpRateLimit, async (req, res) => {
  try {
    ...
    const projects = memberships.map(({ project }) => {
      ...
      const decisionTypeCoverage =
        docCount === 0
          ? 0
          : Math.round(
              (project.documents.filter((document) => Boolean(document.decisionType)).length / docCount) * 100,
            );
...
router.get('/api/v1/projects/:projectId/search', requireMvpAuth, mvpRateLimit, async (req, res) => {
