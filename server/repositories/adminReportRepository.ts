...
export async function getDbAnalysis(): Promise<DbAnalysisResponse> {
  ...
  return {
    generatedAt: new Date().toISOString(),
    requirements: {
      byCategory: reqByCategory.map((r: any) => ({ category: String(r.category), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byCodingConfidence: reqByCodingConfidence.map((r: any) => ({ confidence: String(r.codingConfidence), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byLevel: reqByLevel.map((r: any) => ({ level: String(r.level), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byStatus: reqByStatus.map((r: any) => ({ status: String(r.statusInNotification), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      municipalitySpecificCount,
      minimumRequirementCount,
      withCitationsCount,
      citationsTotal,
    ...
    documents: {
      byStatus: docByStatus.map((r: any) => ({ status: String(r.status), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byDecisionType: docByDecisionType.map((r: any) => ({ decisionType: String(r.decisionType ?? "(okänd)"), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byLegalStatus: docByLegalStatus.map((r: any) => ({ legalStatus: String(r.legalStatus ?? "(okänd)"), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
    ...
    extractedRequirements: {
      byCategory: extByCategory.map((r: any) => ({ category: String(r.category), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
      byLevel: extByLevel.map((r: any) => ({ level: String(r.requirementLevel), count: Number(r._count._all) })).sort((a, b) => b.count - a.count),
  ...
  return {
    ...
    pipelineRuns: {
      ...
      rows: pipeRows.map((r: any) => ({
        ...
        requirementsExtracted: r.errorCount ?? null,
