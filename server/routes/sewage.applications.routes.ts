...
function toPublicApplication(record: ReturnType<typeof getSewageApplicationById>) {
...
function requireApplicationAccess(
  record: NonNullable<ReturnType<typeof getSewageApplicationById>>,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (patch.projectId) {
        await assertProjectAccess(req.authUser, patch.projectId);
      }

      const result = patchApplicationDraft(id, patch);
      if (!result.ok) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (body.projectId) {
        await assertProjectAccess(req.authUser, body.projectId);
      }
      ...
      const result = await submitSewageApplication(id, req.authUser, {
        ...
        protectionProfile: body.protectionProfile as Parameters<typeof submitSewageApplication>[2]['protectionProfile'],
      ...
      await auditTrail.logSubmission(
        result.referenceNumber,
        id,
        req.authUser.id,
        result.municipalityCode,
        ['situationPlan', 'crossSection'],
      );
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const updated = updateSewageApplicationRecord(id, {
...
router.get('/api/sewage/applications/:id/export', requireAuth, rateLimitByUser(20, 60_000), async (req, res) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
