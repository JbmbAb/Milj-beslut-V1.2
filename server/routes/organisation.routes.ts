...
import { createInvitation, listInvitations, acceptInvitation, revokeInvitation } from "../services/orgInvitationService";
...
router.delete("/api/orgs/:orgId/invitations/:inviteId", requireAuth, rateLimitByUser(20, 60_000), async (req, res) => {
  try {
    ...
    await revokeInvitation({ orgId, inviteId: routeParam(req.params.inviteId), actingUserId: req.authUser.id });
    res.json({ ok: true });
  } catch (error: unknown) {
    res.status(400).json(toSafeErrorResponse(error));
  }
