...
router.get('/api/datasources/health', rateLimitByUser(30, 60_000), async (_req, res, next) => {
  try {
...
router.post('/api/datasources/open/sync', requireAuth, rateLimitByUser(10, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
...
router.get('/api/datasources/slu/status', requireAuth, rateLimitByUser(10, 60_000), (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.post('/api/datasources/slu/proxy', requireAuth, rateLimitByUser(20, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.get('/api/search/status', requireAuth, rateLimitByUser(30, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.get('/api/search/status/:projectId', requireAuth, rateLimitByUser(30, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.post('/api/search/recover-stale', requireAuth, rateLimitByUser(6, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.post('/api/search/retry-failed', requireAuth, rateLimitByUser(10, 60_000), async (req, res, next) => {
  try {
    if (!req.authUser) {
      res.status(401).json({ ok: false, error: 'Unauthorized' });
      return;
...
router.get('/api/geo/markcover', requireAuth, rateLimitByUser(40, 60_000), async (req, res, next) => {
  try {
...
router.get('/api/geo/terrain', requireAuth, rateLimitByUser(30, 60_000), async (req, res, next) => {
  try {
    const bboxStr = String(req.query.bbox ?? '');
    const bbox = parseBbox(bboxStr);
    if (!bbox) {
