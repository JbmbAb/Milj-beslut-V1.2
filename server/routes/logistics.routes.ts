...
import { addGpsPosition, getGpsTrack, getLatestPosition as getLatestGpsPosition } from "../../legacy/experimental/gpsTrackingService";
import { getMarketSnapshot, invalidateMarketCache } from "../../legacy/experimental/marketIntelService";
import * as transportService from "../services/transportDispatchService";
import * as limsService from "../services/limsService";
...
router.post("/api/projects/:projectId/transport/:bookingId/gps/update", requireAuth, rateLimitByUser(120, 60_000), async (req, res) => {
  try {
    ...
    const { lat, lng, altitude, speedKmh, heading, accuracy } =
      req.body as { lat?: number; lng?: number; altitude?: number; speedKmh?: number; heading?: number; accuracy?: number; };
    ...
    const position = await addGpsPosition({
      ...
      lat, lng, altitude, speedKmh, heading, accuracy,
      actingUserId: req.authUser.id,
    });
    res.json({ ok: true, position });
  } catch (error: unknown) { res.status(400).json(toSafeErrorResponse(error)); }
});

router.get("/api/projects/:projectId/transport/:bookingId/gps", requireAuth, rateLimitByUser(60, 60_000), async (req, res) => {
  try {
...
router.get("/api/projects/:projectId/transport/:bookingId/gps/latest", requireAuth, rateLimitByUser(120, 60_000), async (req, res) => {
  try {
...
router.get("/api/transport/bookings/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (!booking) { res.status(404).json({ ok: false, error: "Bokning saknas" }); return; }
    res.json({ ok: true, booking });
  } catch (error: unknown) { res.status(400).json(toSafeErrorResponse(error)); }
});
