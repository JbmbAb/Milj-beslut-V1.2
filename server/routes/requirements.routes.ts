...
import { getDocumentById as getSearchDocumentById } from "../repositories/searchRepository";
...
router.patch("/api/admin/requirements/cases/:caseId/review", requireAuth, rateLimitByUser(50, 60_000), async (req, res) => {
  try {
    if (!req.authUser || req.authUser.role !== "ADMIN") {
      res.status(403).json({ ok: false, error: "Admin role required" });
      return;
    ...
    if (caseReviewStatus !== "AUTO" && !validatedBy) {
      res.status(400).json({ ok: false, error: "validatedBy is required when setting a manual case review status" });
      return;
    }
...
router.patch("/api/admin/requirements/citations/:citationCode/verify", requireAuth, rateLimitByUser(50, 60_000), async (req, res) => {
  try {
    if (!req.authUser || req.authUser.role !== "ADMIN") {
      res.status(403).json({ ok: false, error: "Admin role required" });
      return;
