...
export async function getSearchWorkerStatus(): Promise<{ pendingJobs: number; failedJobs: number }> {
  try {
    const [pendingJobs, failedJobs] = await Promise.all([
      prisma.searchJob.count({ where: { status: 'PENDING' } }),
      prisma.searchJob.count({ where: { status: 'FAILED' } }),
    ]);
