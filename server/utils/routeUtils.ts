import { parseBbox } from "../services/publicUiService";
import { RequirementCaseReviewStatus, RequirementVerificationStatus } from "../repositories/requirementsRepository";
...
export const requirementStatuses: RequirementVerificationStatus[] = ["AUTO", "REVIEWED", "VERIFIED", "REJECTED"];
export const requirementCaseReviewStatuses: RequirementCaseReviewStatus[] = ["AUTO", "NEEDS_REVIEW", "VERIFIED", "LOCKED"];
...
export function parseOptionalRequirementCaseReviewStatus(value: unknown): RequirementCaseReviewStatus | undefined {
  const normalized = firstValue(value);
  if (typeof normalized !== "string") return undefined;
