...
export function summarizeExternalHealthReport(checks: ExternalHealthCheck[], checkedAt: string = nowIso()): ExternalHealthReport {
...
function integrationCategory(key?: string): string {
  ...
  if (key === "riksantikvarieambetet" || key === "naturvardsverket" || key === "sgu" || key === "msb" || key === "boverket" || key === "hav" || key === "lansstyrelsen") {
    return "Datakallor";
  }
...
function mapOpenProbeToCheck(source: (typeof SOURCE_CATALOG)[number], probe: OpenSourceProbe | undefined): ExternalHealthCheck {
...
async function probeOpenAi(): Promise<ExternalHealthCheck> {
  ...
  if (!response.ok) {
    ...
    return buildCheck({
      ...
      mode: "live",
      configured: true,
      detail: `OpenAI svarade ${response.status}: ${text.slice(0, 140)}`,
      endpoint: "https://api.openai.com/v1/models",
  ...
  return buildCheck({
    ...
    mode: "live",
    configured: true,
    detail: `Livecheck OK. Modellprov: ${payload.data?.[0]?.id || "okand modell"}.`,
    endpoint: "https://api.openai.com/v1/models",
    responseCode: response.status,
    activation: "OPTIONAL",
  });
...
async function probeGemini(): Promise<ExternalHealthCheck> {
  ...
  if (!response.ok) {
    ...
    return buildCheck({
      ...
      mode: "live",
      configured: true,
      detail: `Gemini svarade ${response.status}: ${text.slice(0, 140)}`,
      endpoint: "https://generativelanguage.googleapis.com/v1beta/models",
      responseCode: response.status,
      activation: "OPTIONAL",
    });
  ...
  return buildCheck({
    ...
    mode: "live",
    configured: true,
    detail: `Livecheck OK. Modellprov: ${payload.models?.[0]?.name || "okand modell"}.`,
    endpoint: "https://generativelanguage.googleapis.com/v1beta/models",
    responseCode: response.status,
    activation: "OPTIONAL",
  });
...
async function probeLantmaterietLicensed(): Promise<ExternalHealthCheck> {
  ...
  const baseUrl = String(process.env.LANTMATERIET_BASE_URL || "https://api.lantmateriet.se/ogc-features/v1").trim();
  ...
  const lookupMode = String(process.env.LANTMATERIET_LOOKUP_MODE || "").trim().toLowerCase();
...
async function probeSlu(): Promise<ExternalHealthCheck> {
  ...
  const okEndpoint = pingResults.find((result): result is PromiseFulfilledResult<{ ok: boolean; status: number; endpoint: string }> => {
...
function probePermitAuthority(): ExternalHealthCheck {
  ...
  if (!endpoint) {
    return buildCheck({
      ...
      mode: "config",
      configured: false,
      detail: "AUTHORITY_SUBMIT_ENDPOINT saknas. Permit-submit ar blockerad tills riktig endpoint ar konfigurerad.",
      activation: "OPTIONAL",
    });
  }
...
export async function getExternalHealthReport(): Promise<ExternalHealthReport> {
  ...
  const specialCatalogKeys = new Set(["lantmateriet_licensed", "bankid", "slu", "kommun_kontakter_csv", "kommunala_diarier"]);
  const openCatalogSources = SOURCE_CATALOG.filter((source) => !specialCatalogKeys.has(String(source.implementationKey || "")));
  ...
  checks.push(
    ...
    probeConfigOnlyIntegration({
      ...
      configuredDetail: "LIMS-endpoint finns, men ingen generell liveprobe goras eftersom kontraktet ar systemspecifikt.",
    }),
    probeConfigOnlyIntegration({
      key: "ocr_api",
      label: "OCR API",
