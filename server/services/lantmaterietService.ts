...
const lantmaterietBreaker = new CircuitBreaker({
  name: 'Lantmateriet',
  failureThreshold: 4,
...
async function tryFastighetsindelningOpenSubscriptionLookup(
  ...
  try {
    const response = await lantmaterietBreaker.execute(() =>
...
export async function getLantmaterietAccessToken(): Promise<string> {
  ...
  console.log(`Lantmäteriet: hämtar token från ${tokenUrl}`);
...
export async function testLantmaterietConnection(): Promise<LantmaterietConnectionTestResult> {
  ...
  try {
    const resp = await lantmaterietBreaker.execute(() =>
      fetch(lookupUrl, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
