...
const OCR_MODEL = process.env.GEMINI_OCR_MODEL || process.env.OCR_MODEL || "gemini-1.5-flash";
...
export async function runGeminiOcr(fileBuffer: Buffer, mimeType: string, modelOverride?: string): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Gemini OCR Response Not OK:', response.status, await response.text());
      return null;
    }
...
export async function loadPdfText(filePath: string, fallbackTitle: string, forceOcr = false): Promise<string> {
...
export async function syncManifestMetadata(input: {
  ...
  for (const row of rows) {
    ...
    const hazardousFlag = parseBooleanOrNull(readField(row, ["Hazardous", "hazardous_flag", "farligt", "farligt_avfall"]));
...
export async function runSearchQuery(input: {
  ...
  const uniqueDesignations = Array.from(new Set(candidates
    .map(c => c.project?.propertyDesignation)
    ...
    .map((candidate) => {
      ...
      return {
        ...
        metadata: {
          ...
          geoRisk: candidate.project?.propertyDesignation ? geoRiskMap.get(candidate.project.propertyDesignation) : null,
        },
      };
    })
