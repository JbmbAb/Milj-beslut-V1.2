...
import { tryFetchLocalSguData } from './hybridGeoService';
...
export async function fetchGeologicalData(lat: number, lng: number): Promise<GeologicalData> {
  ...
  // 0. Try local PostGIS first
  try {
    const localSgu = await tryFetchLocalSguData(lat, lng);
    if (localSgu) {
      result.soilType = localSgu.jg2_tx || localSgu.jordart || 'Information saknas';
      result.coverageMode = 'complete';
    }
  } catch (err) {
    logger.warn('SGU local lookup failed, falling back to OGC', { err });
  }
  ...
  // 1. Fetch Soil Types (Jordarter) - only if still unknown
  if (result.soilType === 'Okänd') {
    try {
