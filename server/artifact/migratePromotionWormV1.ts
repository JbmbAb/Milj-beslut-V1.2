import { createApprovalRecord, type ApprovalDecision, type ApprovalRecord } from './ApprovalRecord';
...
export type WormMigrationSummary = {
  readonly approvedMigrated: number;
  readonly rejectedArchived: number;
 ...
 *
 * - promotion-approved/* → reconstruct ApprovalRecord + PromotionArtifactV3 → promotion/{artifactId}
 * - promotion/* absent from promotion-approved → legacy-rejected-promotion/{id}
 *
 * Never copy rejected v2 promotions into the new promotion/ namespace.
 */
export async function migratePromotionWormV1(
  ...
  for (const key of promotionKeys) {
    ...
    // Skip content-addressed sha256 keys already written as V3
  ...
  return { approvedMigrated, rejectedArchived, skipped, dryRun, writtenKeys };
}
...
function rebuildApprovedV3(
  ...
  const decision: ApprovalDecision = v2.approvalDecision ?? {
    approved: true,
  ...
  const approval = createApprovalRecord({
    ...
    createdAt: decision.timestamp ?? v2.promotedAt,
  });
