...
function hasWindow(): boolean {
  // In Vitest/unit tests we run in JSDOM (window exists) but still want server code paths to behave like Node.
...
export async function serverGenerateText(prompt: string): Promise<string | null> {
  ...
  try {
        const model = client.getGenerativeModel({ model: "gemini-2.0-flash", systemInstruction: GEMINI_SYSTEM_PROMPT });
...
function weatherLevelFromMunicipality(municipality: string): WeatherRisk["level"] {
  ...
  if (key.includes("goteborg") || key.includes("göteborg") || key.includes("malmo") || key.includes("malmö")) return "Medel" as WeatherRisk["level"];
...
export const analyzePermitRisk = async (permit: Permit): Promise<string> => {
  ...
  const basis = permit.decision_type === "AVSLAG" ? "högre regulatorisk risk" : "normal regulatorisk risk";
  return `Offline-analys: Ärendet bedöms ha ${basis}. Prioritera kontroll av lagringstid, skyddsavstånd och dokumenterad egenkontroll.`;
};
...
export const generatePlanDraft = async (
  ...
  if (type === "background") {
        return `Projektet avser ${context || "angiven verksamhet"} och syftar till att uppfylla myndighetskrav med tydlig styrning, kontroll och uppföljning.`;
  }
  if (type === "description") {
        return `Genomförande sker etappvis med fokus på riskminimering, verifierad dokumentation och spårbar leverans mot stage-gates.`;
  }
    return "1. Kvalitetssäkrad datainsamling. 2. Riskbaserad prioritering. 3. Spårbar rapportering till berörda intressenter.";
};
...
export const predictWeatherRisk = async (municipality: string): Promise<WeatherRisk> => {
  ...
  if (serverResult) {
    ...
    return { level, description: safeSnippet(serverResult, 180), action: "Planera erosionsskydd och uppföljning av nederbörd." };
  }

  const level = weatherLevelFromMunicipality(municipality);
  return {
    level,
    description: `Offline-prognos för ${municipality}: normal drift med behov av daglig kontroll av nederbörd och avrinning.`,
    action: "Säkerställ avvattning, invallning och uppdaterad väderrutin i arbetsberedning.",
  };
};
...
export const generateFigmaAiResponse = async (
  ...
  if (style === "detailed") {
    ...
    return `Bygg en tydlig vy med status först, handlingar sedan och detaljer sist.`;
};
...
`;
  ...
  return {
    ...
    transport_requirements: [],
    environmental_risks: ["Okänd risk (Offline)"],
