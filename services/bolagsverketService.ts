...
export class BolagsverketService {
  ...
  constructor() {
    ...
    this.baseUrl =
      process.env.BOLAGSVERKET_API_URL || 'https://gw.api.bolagsverket.se/vardefulla-datamangder/v1';
  ...
  async getCompanyInfo(orgNumber: string): Promise<any> {
    try {
      ...
      console.log(
        `Saknar BOLAGSVERKET_API_KEY eller anropet misslyckades. Använder öppet API för ${cleanOrgNr}...`,
      );
   ...
   */
  async getSignatories(_orgNumber: string): Promise<any> {
    if (!this.apiKey) {
      return {
        status: 'pending_auth',
