    ...
    <div className="space-y-6 h-full flex flex-col">
      {!summary && !loading && (
        <div className="text-center py-12 flex-1 flex flex-col justify-center">
           ...
           <p className="text-slate-500 text-sm max-w-xs mx-auto mb-8">Vi kör en kombinerad analys på lokala beslut och nationella marknadstrender.</p>
           <button 
      ...
      {loading && (
        <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-4">
           ...
           <p className="text-xs font-black text-indigo-400 uppercase tracking-widest animate-pulse">Analyserar 1577 pusselbitar...</p>
        </div>
      )}

      {summary && (
        <div className={`animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col ${fullView ? 'bg-slate-900 p-10 rounded-[3rem]' : ''}`}>
           <div className="flex justify-between items-center mb-6">
              <h4 className="text-indigo-400 font-black text-xs uppercase tracking-[0.2em]">Marknadsrapport: Genererad Nu</h4>
              <button onClick={() => setSummary(null)} className="text-slate-600 hover:text-indigo-400 transition-colors">
           ...
           {summary.sources.length > 0 && (
             <div className="space-y-3">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Web Grounding Källor</p>
                <div className="flex flex-wrap gap-2">
                   {summary.sources.map((s, i) => s.web && (
                     <a key={i} href={s.web.uri} target="_blank" className="px-3 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-[10px] text-indigo-400 flex items-center gap-2">
