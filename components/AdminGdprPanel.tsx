...
const AdminGdprPanel: React.FC = () => {
  ...
  const clearStatus = () => { setError(''); setInfo(''); };
  ...
  const handleDeleteUser = async () => {
    if (!deleteUserId.trim()) { setError('Ange ett användar-ID'); return; }
    if (!deleteConfirm) { setError('Bekräfta radering genom att markera checkboxen'); return; }
    clearStatus();
    setBusy('delete');
    try {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Status banner */}
      {(error || info) && (
        <div data-testid="gdpr-status-banner" className="rounded-xl border border-slate-200 bg-white p-3 text-xs">
          {error && <p data-testid="gdpr-status-error" className="font-bold text-rose-600">{error}</p>}
          {info && <p data-testid="gdpr-status-info" className="text-slate-700">{info}</p>}
        </div>
      )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <span className="rounded-full bg-blue-100 px-2 py-1 text-[10px] font-black uppercase text-blue-700">Art. 20</span>
        ...
        {exportResult && (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700 space-y-1">
            <p><span className="font-bold">Användar-ID:</span> {exportResult.userId}</p>
            {exportResult.email && <p><span className="font-bold">E-post:</span> {exportResult.email}</p>}
            {exportResult.name && <p><span className="font-bold">Namn:</span> {exportResult.name}</p>}
            <p><span className="font-bold">Exportdatum:</span> {new Date(exportResult.exportedAt).toLocaleString('sv-SE')}</p>
            <p><span className="font-bold">Projekt:</span> {exportResult.projects?.length ?? 0} st</p>
            <p><span className="font-bold">Dokument:</span> {exportResult.documents?.length ?? 0} st</p>
            <p><span className="font-bold">Händelser:</span> {exportResult.auditEntries?.length ?? 0} st</p>
          </div>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-rose-100 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <span className="rounded-full bg-rose-100 px-2 py-1 text-[10px] font-black uppercase text-rose-700">Art. 17</span>
          <h3 className="font-black text-slate-900">Radera användares personuppgifter</h3>
          <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-700">ADMIN</span>
        </div>
        <p className="text-xs text-slate-600 mb-4">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black uppercase text-slate-700">Underhåll</span>
          <h3 className="font-black text-slate-900">GDPR-underhåll</h3>
          <span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-700">ADMIN</span>
        </div>
        <p className="text-xs text-slate-600 mb-4">
          Kör periodisk rensning: utgångna sessioner, anonymisering av gammal händelselogg och rensning av mjukt raderade poster.
        </p>
        <button
        ...
        {maintenanceResult && (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700 space-y-1">
            {maintenanceResult.deletedExpiredSessions !== undefined && (
              <p><span className="font-bold">Raderade utgångna sessioner:</span> {maintenanceResult.deletedExpiredSessions}</p>
            )}
            {maintenanceResult.anonymizedOldAuditEntries !== undefined && (
              <p><span className="font-bold">Anonymiserade händelseposter:</span> {maintenanceResult.anonymizedOldAuditEntries}</p>
            )}
            {maintenanceResult.purgedSoftDeleted !== undefined && (
              <p><span className="font-bold">Rensade mjukt raderade:</span> {maintenanceResult.purgedSoftDeleted}</p>
            )}
          </div>
        )}
      </section>
      ...
      <p className="text-[11px] text-slate-400">
        Alla GDPR-åtgärder loggas i revisionsloggen (audit trail) för spårbarhet.
        Kräver admin-inloggning via Admin sökcenter.
      </p>
    </div>
  );
};
