...
import { Search, Rocket, FileText, AlertTriangle, ChevronRight, LayoutDashboard, Download, ClipboardCheck, Activity, ShieldAlert, Scale } from 'lucide-react';
...
const Card: React.FC<{ children: React.ReactNode; className?: string; onClick?: () => void }> = ({ children, className = "", onClick }) => (
    <div className={`rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden ${className}`} onClick={onClick}>
        {children}
    </div>
);

const Badge: React.FC<{ label: string; color?: string; className?: string; icon?: React.ReactNode }> = ({ label, color = "bg-slate-100 text-slate-700", className = "", icon }) => (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black tracking-tight ${color} ${className}`}>
        {icon && <span>{icon}</span>}
        {label}
    </span>
);
                    ...
                    <input
                        ...
                        onKeyDown={e => {
                            if (e.key === 'Enter') {
                                ...
                                if (val) onSelect({ id: 'new-demo-project', propertyDesignation: val, status: 'ACTIVE', docCount: 0, coverage: { municipality: 0, decisionType: 0 } } as any);
                            }
                        }}
                    />
                </div>
                <button
                    onClick={() => onSelect({ id: 'new-demo-project', propertyDesignation: 'KOMMUNVÄGEN 1:1', status: 'ACTIVE', docCount: 0, coverage: { municipality: 0, decisionType: 0 } } as any)}
                    className="px-4 py-2 bg-slate-900 text-white text-sm font-black rounded-xl hover:bg-black transition"
                >
                    Starta nytt projekt
                </button>
            </div>
                    ...
                    <Card key={project.id} className="hover:border-indigo-300 transition-colors cursor-pointer group" onClick={() => onSelect(project)}>
                        <div className="p-5">
                            <div className="flex justify-between items-start mb-4">
                                <Badge label={project.status} color={project.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-600'} />
                            ...
                            <div className="mt-5 space-y-3">
                                <div className="space-y-1">
                                    ...
                                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${project.coverage.municipality}%` }} />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <div className="flex justify-between text-[11px] font-bold uppercase text-slate-400">
                                    ...
                                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${project.coverage.decisionType}%` }} />
                                    </div>
                                </div>
                            </div>
                        </div>
...
const MunicipalityInsightCard: React.FC<{ name: string }> = ({ name }) => {
    ...
    useEffect(() => {
        if (!name) return;
        callMvp<{ insight: MunicipalityInsight }>(`/api/v1/municipality/${name}/insight`, { method: 'GET' })
        ...
        <Card className="p-6 bg-white border-indigo-100 shadow-sm">
            <div className="flex justify-between items-start mb-6">
                ...
                <div className="text-right">
                    ...
                    <div className="text-[9px] font-bold text-slate-400 uppercase">Ranking: #{insight.ranking} av 290</div>
                </div>
            </div>
                ...
                {insight.patterns && insight.patterns.length > 0 && (
                    <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {insight.patterns.map(p => <Badge key={p} label={p} color="bg-amber-50 text-amber-700 border border-amber-100" />)}
                        </div>
                    </div>
                )}
                <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {insight.commonRisks.map(r => <Badge key={r} label={r} color="bg-indigo-50 text-indigo-700" />)}
                    </div>
                </div>
                <div>
                    <h4 className="text-[9px] font-black text-slate-400 uppercase mb-2">Typiska Krav i {insight.name}</h4>
                    <div className="flex flex-wrap gap-1.5">
                        {insight.commonRequirements.map(r => <Badge key={r} label={r} color="bg-emerald-50 text-emerald-700" />)}
                    </div>
                </div>
            </div>
        </Card>
...
const DocumentSearch: React.FC<{ project: Project }> = ({ project }) => {
    ...
    const performSearch = async () => {
        ...
        setLoading(true);
        try {
            const res = await callMvp<any>(`/api/v1/projects/${project.id}/search`, {
                method: 'GET',
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-400">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                ...
                <p className="text-sm text-slate-500 mb-6">Hybrid-sökning (semantisk + nyckelord) mot RAG-indexerade dokument för {project.propertyDesignation}.</p>
                ...
                {results.map((res, idx) => (
                    <Card key={idx} className="p-5 border-l-4 border-l-indigo-500">
                        <div className="flex justify-between items-start mb-2">
                            <h4 className="font-bold text-slate-900 underline decoration-slate-200 hover:decoration-indigo-400 cursor-pointer">{res.originalName || res.subject}</h4>
...
const ClassificationPanel: React.FC<{ project: Project }> = ({ project }) => {
    ...
    const runClassification = async () => {
        setLoading(true);
        try {
            const res = await callMvp<Classification>('/api/v1/classification', {
                method: 'POST',
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-400">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Bestäm verksamhetstyp och risknivå baserat på befintliga beslut.</p>
                </div>
                {!data && (
                    <button
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <Badge
                                    ...
                                    color={data.riskLevel === 'HIGH' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}
                                    className="px-3"
                                />
                            </div>
                            <div className="flex items-center justify-between">
                        ...
                        {data.missingFields.length > 0 && (
                            <div className="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {data.missingFields.map(f => <Badge key={f} label={f} color="bg-white text-amber-700 border border-amber-200" />)}
                                </div>
                            </div>
                        )}
                    </Card>
...
const PermitGenerator: React.FC<{ project: Project }> = ({ project }) => {
    const [formData, setFormData] = useState({
        ...
        ewcCode: '17 05 04',
        wasteDescription: 'Osorterat schaktavfall från infrastrukturprojekt.',
        volume: '1500',
        hazardous: false,
        waterHandling: 'Dagvatten leds till oljeavskiljare och därefter till recipient.',
        storageMethod: 'Massor lagras i tätade fack på hårdgjord yta under tak.'
    });
    ...
    useEffect(() => {
        ...
        setFormData(prev => ({
            ...prev,
            property: project.propertyDesignation,
            municipality: project.id === 'new-demo-project' ? muni : (project.coverage.municipality > 0 ? project.propertyDesignation.split(' ')[0] : prev.municipality)
        }));
    }, [project]);
    ...
    const generate = async () => {
        setLoading(true);
        try {
            const res = await callMvp<any>('/api/v1/permit/generate', {
                method: 'POST',
                body: {
                    project_data: {
            ...
            <div className="space-y-6">
                <Card className="p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="text-xs font-black text-slate-400 uppercase mb-1 block">Hantering av vatten</label>
                            <textarea
                                rows={3}
                ...
                {result ? (
                    <div className="space-y-6 animate-in fade-in duration-700">
                        <header className="flex items-center justify-between">
                            <Badge label="Genererat Utkast" color="bg-indigo-600 text-white px-3 py-1.5" />
                            <button
                    ...
                    <div className="h-full min-h-[500px] flex flex-col items-center justify-center bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center">
                        ...
                        <p className="max-w-xs text-sm text-slate-400 mt-2">Fyll i formuläret och tryck på kör för att skapa ett AI-understött anmälningsdokument.</p>
                    </div>
                )}
            </div>
        </div>
...
export const MvpDemoInterface: React.FC = () => {
    ...
    const [view, setView] = useState<'dashboard' | 'search' | 'classify' | 'generate'>('dashboard');
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 font-['Plus_Jakarta_Sans']">
                ...
                <div className="flex items-center gap-2">
                    {activeProject ? (
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Aktivt Projekt</p>
                                <p className="text-sm font-black text-slate-900 leading-tight">{activeProject.propertyDesignation}</p>
                            </div>
                        </div>
                    ) : (
                        <h1 className="text-lg font-black text-slate-900 tracking-tight">Miljöbeslut.se <span className="text-indigo-600">MVP</span></h1>
                    )}
                </div>
                ...
                <div className="flex gap-1 bg-slate-100 p-1.5 rounded-2xl">
                    {navItems.map(item => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all ${view === item.id
                                ? 'bg-white text-slate-900 shadow-sm'
                                : (item.disabled ? 'text-slate-300 cursor-not-allowed' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50')
                                }`}
                        >
            ...
            <main className="flex-1 overflow-y-auto w-full px-8 py-8">
                <div className={`max-w-7xl mx-auto grid gap-8 ${activeProject ? 'lg:grid-cols-[1fr_320px]' : 'grid-cols-1'}`}>
                    <div className="space-y-8">
                        {view === 'dashboard' && <ProjectDashboard onSelect={p => { setActiveProject(p); setView('search'); }} />}
                    ...
                    {activeProject && (
                        <aside className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                            <div className="text-xs font-black text-slate-400 uppercase tracking-widest">Regulatorisk Insikt</div>
                            <MunicipalityInsightCard key={activeProject.propertyDesignation.split(' ')[0]} name={activeProject.propertyDesignation.split(' ')[0]} />

                            <Card className="p-5 bg-slate-900 text-white border-none">
                                <div className="flex items-center gap-3 mb-4">
                                    <Activity size={18} className="text-indigo-400" />
                                    <h4 className="text-sm font-black">Status i {activeProject.propertyDesignation.split(' ')[0]}</h4>
                                </div>
                                <p className="text-xs text-slate-400 mb-4">
                                    Kommunen har nyligen skärpt tillsynen kring schaktmassor nära vattenskyddsområden.
                                </p>
                                <Badge label="Nytt villkor" color="bg-rose-500/20 text-rose-300" className="w-fit" />
                            </Card>
                        </aside>
                    )}
                </div>
