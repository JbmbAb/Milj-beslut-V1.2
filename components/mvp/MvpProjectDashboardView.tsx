...
const ProjectDashboardView: React.FC<ProjectDashboardViewProps> = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    void callMvp<{ projects: Project[] }>('/api/v1/projects', { method: 'GET' })
      ...
      .catch((loadError) => {
        ...
        if (!cancelled) {
          const message = loadError instanceof Error ? loadError.message : 'Kunde inte ladda projekt.';
          setError(
            /bearer token|unauthorized|invalid bearer/i.test(message)
              ? 'MVP kraver en giltig admin- eller BankID-session for att ladda riktiga projekt.'
              : message,
          );
        }
      })
  ...
  const filteredProjects = projects.filter((project) =>
    project.propertyDesignation.toLowerCase().includes(normalizedQuery),
  );
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 space-y-6 fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500">Ã–versikt av pÃ¥gÃ¥ende miljÃ¶prÃ¶vningar och data-tÃ¤ckning.</p>
      </div>
      <div className="flex items-center gap-3">
        ...
        >
          Nytt projekt krÃ¤ver riktig datakÃ¤lla
        </button>
      </div>
      ...
      {!error && projects.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-slate-500">
          Inga projekt hittades for ditt konto annu.
        </div>
      ) : null}

      {!error && projects.length > 0 && filteredProjects.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-slate-500">
          Ingen fastighet matchar "{query}".
        </div>
      ) : null}
