...
export const TechnicalSluExpert: React.FC = () => {
    ...
    const handleScan = async () => {
        setLoading(true);
        try {
            const result = await analyzeBiodiversity(parseFloat(lat), parseFloat(lng));
            setData(result);
        } catch (e) {
            console.error(e);
            setData({
                summary: 'Offline-läge: Inventering kunde inte hämtas från extern källa. Gör manuell kontroll mot lokala naturvärden.',
                observations: [
                    { name: 'Åkergroda', status: 'Fridlyst', distance: 180 },
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex flex-col items-end gap-1 mr-4">
                            <button
                                onClick={() => { setLat('59.8979'); setLng('17.6329'); }}
                                className="text-[9px] font-black uppercase tracking-widest text-amber-500/40 hover:text-amber-500 transition-colors"
                            >
                                Preset: Gamla Uppsala
                            </button>
                            <button
                                onClick={() => { setLat('59.3524'); setLng('18.0433'); }}
                                className="text-[9px] font-black uppercase tracking-widest text-emerald-500/40 hover:text-emerald-500 transition-colors"
                            >
                                Preset: Hagaparken
                            </button>
                        </div>
                        ...
                        <button
                            onClick={handleScan}
                            disabled={loading}
                            className={`px-8 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center gap-2 transition-all duration-300 shadow-xl ${loading
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="p-5 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
                                            <div className="flex items-center gap-3 mb-3">
                                                ...
                                                <h5 className="text-xs font-black uppercase tracking-widest text-amber-500/80">Geologi</h5>
                                            </div>
                                            <p className="text-sm font-bold text-slate-200">{data.geological.soilType}</p>
                                            <p className="text-[10px] text-slate-500 mt-1 uppercase font-medium">Huvudsaklig jordart</p>
                                        </div>
                                        <div className="p-5 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
                                            <div className="flex items-center gap-3 mb-3">
                                                <Waves size={18} className="text-blue-500" />
                                                <h5 className="text-xs font-black uppercase tracking-widest text-blue-500/80">Grundvatten</h5>
                                            </div>
                                            <p className="text-sm font-bold text-slate-200">{data.geological.groundwaterVulnerability}</p>
                            ...
                            <div className="space-y-4">
                                ...
                                {data.compliance && (
                                    <div className="p-6 rounded-2xl bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-white/5 space-y-6">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Riskklassning</p>
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-xl font-black font-['Outfit'] ${data.compliance.overallRisk === 'BLOCK' ? 'text-rose-500' :
                                                        data.compliance.overallRisk === 'HIGH' ? 'text-orange-500' :
                                                            data.compliance.overallRisk === 'MEDIUM' ? 'text-amber-400' :
                                                                'text-emerald-500'
                                                        }`}>
                                        ...
                                        <div className="space-y-3">
                                            {data.compliance.rules.map((rule, i) => (
                                                <div key={i} className="flex gap-3 items-start">
                                                    <div className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${rule.risk === 'BLOCK' ? 'bg-rose-500' :
                                                        rule.risk === 'HIGH' ? 'bg-orange-500' :
                                                            'bg-amber-500'
                                                        }`} />
                                                    <div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {data.observations.map((obs, i) => (
                                        <div key={`obs-${i}`} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl group hover:bg-white/10 transition-colors">
                                            <div>
                                            ...
                                            <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase ${obs.status.includes('Röd') ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' :
                                                obs.status.includes('Frid') ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
                                    ...
                                    {data.protectedAreas.map((area, i) => (
                                        <div key={`area-${i}`} className="flex items-center justify-between p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl group hover:bg-indigo-500/10 transition-colors">
                                            <div>
                                                <p className="text-sm font-bold text-indigo-200">{area.name}</p>
                                                <p className="text-[9px] font-medium text-indigo-400 uppercase">
                                                    {area.type}
                                                </p>
                                            </div>
                                    ...
                                    {data.monuments?.map((mon, i) => (
                                        <div key={`mon-${i}`} className="flex items-center justify-between p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl group hover:bg-amber-500/10 transition-colors">
                                            <div>
                                                <p className="text-sm font-bold text-amber-200">{mon.name}</p>
                                                <p className="text-[9px] font-medium text-amber-500 uppercase">
                                                    {mon.type}
                                                </p>
                                            </div>
                                    ...
                                    {data.observations.length === 0 && data.protectedAreas.length === 0 && data.monuments?.length === 0 && (
                                        <p className="text-xs text-slate-600 italic p-4">Inga restriktioner funna i närområdet.</p>
                                    )}
                                </div>
                            </div>
                        </motion.div>
