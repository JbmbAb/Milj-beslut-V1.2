...
const RequirementChecklist: React.FC<RequirementChecklistProps> = ({ code, citations = [] }) => {
    ...
    return (
        <div className="rounded-2xl border border-white/15 bg-white/5 p-4 text-white">
            <p className="text-[11px] uppercase tracking-[0.18em] text-blue-300 font-black mb-4">Checklista for regelefterlevnad</p>

            <div className="space-y-4">
                <ChecklistItem
                    label="Lagringstid"
                    value={code.requirements.storageTime || 'Max 1 ar inomhus, 3 ar deponi'}
                ...
                <ChecklistItem
                    label="Maxmangd"
                    value={code.requirements.maxAmount || 'Ej begransad upp till tillstandsnivan'}
                    reference={code.requirements.legalReference}
                    citation={getCitationForLabel('Maxmangd')}
                    isHovered={hoveredCitation === 'Maxmangd'}
                    onHover={() => setHoveredCitation('Maxmangd')}
                    onLeave={() => setHoveredCitation(null)}
                />
                <ChecklistItem
                    label="Skyddsavstand"
                    value={code.requirements.safetyDistance || 'Standardkrav enligt gellande praxis'}
                    reference={code.requirements.legalReference}
                    citation={getCitationForLabel('Skyddsavstand')}
                    isHovered={hoveredCitation === 'Skyddsavstand'}
                    onHover={() => setHoveredCitation('Skyddsavstand')}
                    onLeave={() => setHoveredCitation(null)}
                />

                {citations.length > 0 && citations.filter(c => !['Lagringstid', 'Maxmangd', 'Skyddsavstand'].includes(c.sourceType)).map((cit, idx) => (
                    <ChecklistItem
                        key={idx}
...
const ChecklistItem: React.FC<ChecklistItemProps> = ({ label, value, reference, citation, isHovered, onHover, onLeave }) => {

    // Mock citation if none provided for MVP demo purposes
    const displayCitation = citation || {
        quoteText: `Enligt aktuella bestammelser maste ${label.toLowerCase()} strikt foljas. ${value}. (Simulerat citat fran vektor DB).`,
            ...
            {isHovered && (
                <div className="absolute z-50 left-full ml-4 top-0 w-80 bg-slate-800 border border-blue-500/30 shadow-2xl rounded-2xl p-4 animate-in fade-in slide-in-from-left-2 duration-200 pointer-events-none">
                    ...
                    <h4 className="text-[10px] font-black uppercase text-blue-400 tracking-widest mb-2 flex items-center gap-2">
                        <i className="fas fa-quote-left"></i> KALLHANVISNING
                    </h4>
                    <p className="text-sm italic text-slate-300 leading-relaxed mb-3">"{displayCitation.quoteText}"</p>
                    <div className="bg-slate-900 rounded-lg p-2 flex justify-between items-center border border-slate-700">
                        <span className="text-xs text-slate-400 font-medium">Kalla: {displayCitation.legalReference}</span>
