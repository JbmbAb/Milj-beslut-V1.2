...
const DbContentsPanel: React.FC<DbContentsPanelProps> = ({ contents, activeTable, onSelectTable }) => {
...
const AdminSearchConsole: React.FC<AdminSearchConsoleProps> = ({ panel = 'search' }) => {
  ...
  // ── App-status auto-poll: fetch immediately when token is set, then every 30s ──
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const json = await response.json();
        if (response.ok && json?.ok) {
          setAppStatus((json as { ok: true; status: AppStatusResponse }).status);
        }
      } catch {
        // best-effort: stale status is acceptable
      }
      // Also fetch completion once per poll cycle (changes rarely)
      try {
        const completionRes = await fetch('/api/admin/completion', {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
        ...
        if (completionRes.ok && completionJson?.ok) {
          setAppCompletion((completionJson as { ok: true; completion: AppCompletionResponse }).completion);
        }
      } catch {
        // best-effort
      }
  ...
  const loadDbContents = async () => {
    ...
    try {
      ...
      setInfo('Databasinnehåll laddat.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Kunde inte läsa databasinnehåll');
    } finally {
      setBusy('');
    }
  };
    ...
    <div className="mx-auto max-w-7xl space-y-6 animate-in fade-in duration-500">
      ...
      {hasActiveSession && (
        ...
        >
          ...
          {completionExpanded && appCompletion && (
              ...
              {appCompletion.categories.map((cat) => (
                ...
                >
                  ...
                  <div className="mt-1.5 text-slate-400">
                    {cat.done} klara
                    {cat.partial > 0 && <>, <span className="text-yellow-600">{cat.partial} delvisa</span></>}
                    {cat.pending > 0 && <>, <span className="text-red-400">{cat.pending} återstår</span></>}
                  </div>
                  {/* Feature list */}
                    ...
                    {cat.features.map((f) => (
                      <li key={f.id} className="flex items-start gap-1.5">
                        ...
                        <span className="text-slate-600">
                          {f.label}
                          {f.note && (
                            <span className="ml-1 text-slate-400 italic"> — {f.note}</span>
                          )}
                        </span>
                      </li>
                    ))}
      ...
      {/* ══════════════════════════════════════════════════════════
          VAD FINNS I DB – tabellbläddring med senaste N poster
      ══════════════════════════════════════════════════════════ */}
      <section data-testid="db-contents-section" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500 font-black">Vad finns i DB</p>
            <h3 className="text-lg font-black text-slate-900">Databasinnehåll — senaste poster per tabell</h3>
            <p className="mt-1 text-xs text-slate-500">
              Visar de 10 senaste raderna i varje nyckelregistertabell för att snabbt verifiera att data verkligen finns.
            </p>
          </div>
          <button
            data-testid="admin-load-db-contents-button"
            className="rounded-xl bg-teal-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-60"
            disabled={Boolean(busy) || !token}
            onClick={loadDbContents}
          >
            {busy === 'dbcontents' ? 'Läser...' : 'Visa innehåll'}
          </button>
        </div>

        {!dbContents && (
          <p className="mt-4 text-sm text-slate-500">
            Klicka "Visa innehåll" för att se de senaste posterna i databasens nyckelregistertabeller.
          </p>
        )}
          ...
          <DbContentsPanel
            ...
            onSelectTable={setDbContentsTable}
          />
        )}
      </section>
