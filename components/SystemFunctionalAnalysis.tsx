...
function FeatureRow({ feature }: { feature: AppFeature }) {
  return (
    <tr className="border-b border-slate-100 last:border-0">
      ...
      <td className="py-1.5 pr-3"><StatusBadge status={feature.status} /></td>
  ...
  return (
    <div className="p-5 max-w-6xl mx-auto space-y-1">
      ...
      {report && (
        <>
          {/* Overall banner */}
          <div className={`rounded-xl p-4 border flex items-center gap-4 mb-4 ${report.overall === 'ok' ? 'bg-emerald-50 border-emerald-200' : report.overall === 'degraded' ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'}`}>
            <i className={`fas text-2xl ${report.overall === 'ok' ? 'fa-circle-check text-emerald-600' : report.overall === 'degraded' ? 'fa-triangle-exclamation text-amber-600' : 'fa-circle-xmark text-red-600'}`} />
            <div className="flex-1">
              <p className="text-sm font-black text-slate-900">
                Systemstatus: <StatusBadge status={report.overall} />
                {' '}· DB-latens: {report.db.latencyMs != null ? `${report.db.latencyMs} ms` : '—'}
                {' '}· Uptime: {Math.floor(report.app.uptimeSeconds / 60)} min
              </p>
              <p className="text-xs text-slate-600 mt-0.5">
                v{report.app.version} · {report.app.environment} · Node {report.app.nodeVersion}
                {' '}· Analys: {new Date(report.generatedAt).toLocaleString('sv-SE')}
              </p>
            </div>
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              {[
                ...
                <div key={stat.label} className="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide mt-0.5">{stat.label}</p>
                </div>
              ))}
            </div>
            ...
            {filteredCategories.map((cat) => (
              <div key={cat.name} className="mb-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-bold text-slate-500">{cat.percent}% ({cat.done}/{cat.total})</span>
                </div>
                <ProgressBar
                  percent={cat.percent}
                  color={cat.percent === 100 ? 'bg-emerald-500' : cat.percent >= 80 ? 'bg-amber-400' : 'bg-slate-400'}
                />
                <div className="mt-2 overflow-x-auto">
                  <table className="w-full text-left">
                    <tbody>
                      {cat.features.map((f) => <FeatureRow key={f.id} feature={f} />)}
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          <Section title="Integrationer" icon="fa-plug">
            {mockIntegrations.length > 0 && (
              <div className="mb-4 p-3 rounded-xl bg-orange-50 border border-orange-200">
                <p className="text-xs font-black text-orange-800 uppercase tracking-wide mb-2">
                  ...
                  {mockIntegrations.length} integrationer kör i mock/ej konfigurerat läge
                </p>
                <ul className="text-xs text-orange-700 space-y-1">
                  {mockIntegrations.map((i) => (
                    <li key={i.name}>
                      <span className="font-bold">{i.name}</span>{i.note ? ` — ${i.note}` : ''}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-orange-700">{mockIntegrations.length} mock/ej konfigurerade</span>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="pb-2 pr-3 text-[10px] font-black text-slate-500 uppercase tracking-wide">Integration</th>
                    <th className="pb-2 pr-3 text-[10px] font-black text-slate-500 uppercase tracking-wide">Status</th>
                    <th className="pb-2 pr-3 text-[10px] font-black text-slate-500 uppercase tracking-wide">Endpoint</th>
                    <th className="pb-2 text-[10px] font-black text-slate-500 uppercase tracking-wide">Notering</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <Section title="Miljövariabler" icon="fa-key">
            <div className="mb-3 flex items-center gap-3">
              <span className="text-xs font-bold text-emerald-700">{report.environment.configured} konfigurerade</span>
              <span className="text-xs text-slate-400">/</span>
              ...
              {report.environment.requiredMissing.length > 0 && (
                <span className="text-xs font-bold text-red-700">
                  ...
                  {report.environment.requiredMissing.length} obligatoriska saknas: {report.environment.requiredMissing.join(', ')}
                </span>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
              {Object.entries(
                report.environment.vars.reduce<Record<string, typeof report.environment.vars>>((acc, v) => {
                  (acc[v.category] = acc[v.category] ?? []).push(v);
                  return acc;
                }, {})
              ).map(([category, vars]) => (
                <div key={category} className="mb-4">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-wide mb-1">{category}</p>
                  <div className="space-y-0.5">
                    {vars.map((v) => (
                      <div key={v.name} className={`flex items-center justify-between rounded-lg px-2 py-1 ${v.configured ? 'bg-emerald-50' : v.required ? 'bg-red-50' : 'bg-slate-50'}`}>
                        <span className="text-[10px] font-mono text-slate-700">{v.name}</span>
                        <span className={`text-[10px] font-bold ${v.configured ? 'text-emerald-700' : v.required ? 'text-red-700' : 'text-slate-400'}`}>
          ...
          <Section title="Databas" icon="fa-database">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-center">
                <p className="text-2xl font-black text-slate-900">{report.database.totalRows.toLocaleString('sv-SE')}</p>
              ...
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-center">
                <p className="text-2xl font-black text-slate-900">{report.database.recentAuditEvents.length}</p>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide">Senaste händelser</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-center">
                <p className={`text-2xl font-black ${report.db.latencyMs != null && report.db.latencyMs < 50 ? 'text-emerald-700' : 'text-amber-700'}`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="pb-2 pr-4 text-[10px] font-black text-slate-500 uppercase tracking-wide">Tabell</th>
                    <th className="pb-2 pr-4 text-[10px] font-black text-slate-500 uppercase tracking-wide">Rader</th>
                    <th className="pb-2 text-[10px] font-black text-slate-500 uppercase tracking-wide">Senaste post</th>
                  </tr>
                </thead>
                <tbody>
                  {report.database.tables.map((t) => (
                    <tr key={t.table} className="border-b border-slate-100 last:border-0">
                      ...
                      <td className="py-1.5 pr-4 text-xs font-bold text-slate-900">{t.rows.toLocaleString('sv-SE')}</td>
          ...
          <Section title="Bakgrundstjänster" icon="fa-gears">
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50">
                <div className="flex items-center gap-3">
                  <span className={`w-2 h-2 rounded-full ${report.backgroundServices.outlookScheduler.running ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  <span className="text-xs font-bold text-slate-800">Outlook-inläsare</span>
                </div>
                <div className="text-right">
                  <StatusBadge status={report.backgroundServices.outlookScheduler.running ? 'LIVE' : 'NOT_CONFIGURED'} />
            ...
            <Section title={`Senaste fel (${report.recentErrors.length})`} icon="fa-bug">
              <div className="space-y-2">
                {report.recentErrors.slice(0, 10).map((err) => (
                  <div key={err.id} className={`p-3 rounded-xl border text-xs ${err.severity === 'ERROR' || err.severity === 'CRITICAL' ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <StatusBadge status={err.severity} />
                      <span className="font-mono text-slate-500 text-[10px]">{err.type}</span>
                      <span className="ml-auto text-[10px] text-slate-400">{new Date(err.capturedAt).toLocaleString('sv-SE')}</span>
                    </div>
