...
import { Trees, Mountain, CloudSnow, Map as MapIcon, Loader2, ShieldCheck } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        const res = await callCore<PropertyAnalysis>(
          `/api/v1/analysis/property/${encodeURIComponent(propertyDesignation)}`,
          {
            method: 'GET',
          },
        );
        ...
        setAnalysis({
          landUse: [
            { objekttyp: 'Skog', area_m2: 45000 },
            { objekttyp: 'Vatten', area_m2: 1200 },
          ],
          soil: [
            { jordart: 'Morän', area_m2: 40000 },
            { jordart: 'Torv', area_m2: 6200 },
          ],
        ...
        <Card className="p-5 flex flex-col bg-slate-950 text-white border-none shadow-xl">
          ...
          {analysis.climate ? (
            <div className="flex-1 flex flex-col justify-center text-center py-4">
              <div className="text-4xl font-black text-indigo-400 mb-1">{analysis.climate.värde}</div>
      ...
      <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-3xl flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-800 mt-1 leading-relaxed">
            Data hämtas och analyseras dynamiskt. Vid avvikelser mellan API-värden och fysiska förhållanden på
            platsen är det utövarens ansvar att verifiera förutsättningarna för Miljöbalkens krav.
          </p>
        </div>
      </div>
    </div>
