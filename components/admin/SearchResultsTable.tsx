...
const SearchResultsTable: React.FC<SearchResultsTableProps> = ({ searchResults }) => {
  ...
  return (
    <section data-testid="admin-search-results" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <tr key={res.documentId} className="align-top hover:bg-slate-50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <span className={`rounded-lg px-2 py-1 text-[10px] font-black ${res.score > 0.8 ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                  ...
                  {res.citations && res.citations.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {res.citations.map((cit, idx) => (
                        <div key={idx} className="rounded border border-indigo-100 bg-indigo-50/50 p-1.5 text-[10px]">
                          <span className="font-bold text-indigo-700">Citat ({Math.round(cit.confidence * 100)}%):</span>{' '}
                          {cit.quote}
                        </div>
                      ))}
                    </div>
                ...
                <td className="px-4 py-3">
                  <div className="space-y-1 text-[10px]">
                    <p><span className="font-bold">Kommun:</span> {res.metadata.municipality || '-'}</p>
                    <p><span className="font-bold">Typ:</span> {res.metadata.wasteType || '-'}</p>
                    <p><span className="font-bold">Beslut:</span> {res.metadata.decisionType || '-'}</p>
                    <p><span className="font-bold">Organisation:</span> {res.metadata.organisationName || '-'}</p>
                    <p><span className="font-bold">Datum:</span> {res.metadata.receivedTime ? new Date(res.metadata.receivedTime).toLocaleDateString('sv-SE') : '-'}</p>
                  </div>
                </td>
              </tr>
            ))}
