    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500 font-black">Övergripande Compliance</p>
            <h3 className="text-lg font-black text-slate-900">{plan.name}</h3>
          </div>
          <div className="text-right">
            <span className={`inline-flex rounded-2xl px-4 py-2 text-3xl font-black ${
              plan.complianceScore >= 80 ? 'bg-emerald-100 text-emerald-800' : 
              plan.complianceScore >= 50 ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'
            }`}>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          <div className="rounded-2xl bg-slate-50 p-4">
            ...
            <p className={`mt-1 text-4xl font-black ${
               scores?.fundingRisk.rating.startsWith('A') ? 'text-emerald-600' :
               scores?.fundingRisk.rating.startsWith('B') ? 'text-amber-600' : 'text-red-600'
            }`}>
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {scores?.regulatoryRisk.topRiskFactors.map((factor, i) => (
              <span key={i} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-600">
                 ⚠️ {factor}
              </span>
            ))}
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-4 text-[10px] text-slate-500">
              ...
              <span>Injunction Prob: {Math.round((scores?.regulatoryRisk.probabilityInjunction || 0) * 100)}%</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-bold">
              ...
              <span className="text-slate-900">{Math.round((scores?.environmentalRisk.score || 0) * 100)}%</span>
            </div>
            <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden">
              <div 
            ...
            <div className="flex gap-4 text-[10px] text-slate-500">
              <span>Grundvatten: {Math.round((scores?.environmentalRisk.groundwaterImpact || 0) * 100)}%</span>
              <span>Natura2000: {Math.round((scores?.environmentalRisk.biodiversityImpact || 0) * 100)}%</span>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-black text-emerald-900 uppercase tracking-wider">Klimatprestanda</p>
