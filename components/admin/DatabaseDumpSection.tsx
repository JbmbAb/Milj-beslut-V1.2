  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.18em] text-slate-500 font-black">Databasinspektion</p>
          <h3 className="text-lg font-black text-slate-900">Rådata-dump från nyckeltabeller</h3>
          <p className="mt-1 text-xs text-slate-500">Dumpa innehållet direkt från Prisma-lagret för att inspektera rådata.</p>
        </div>
        <button
      ...
      {dbDump && (
        <div className="mt-5 space-y-4">
          <p className="text-xs text-slate-500">Genererad: {new Date(dbDump.generatedAt).toLocaleString('sv-SE')}</p>
          ...
          <p className="text-xs font-black uppercase tracking-widest text-slate-500 mt-6">Tabellinnehåll (Provurval)</p>
