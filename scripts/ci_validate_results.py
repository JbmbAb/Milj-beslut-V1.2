...
def validate(
    ...
    *,
    ...
    ndcg_tol = (
        ndcg_tolerance
        if ndcg_tolerance is not None
        else float(os.environ.get("NDCG_REGRESSION_TOLERANCE", "0.02"))
    )
    lat_tol = (
        latency_tolerance
        if latency_tolerance is not None
        else float(os.environ.get("LATENCY_REGRESSION_TOLERANCE", "0.20"))
    )
    max_fail = (
        hard_failure_rate
        if hard_failure_rate is not None
        else float(os.environ.get("HARD_FAILURE_RATE", "0.05"))
    )
    ...
    ndcg_ci = (
        ci.get("ndcg10")
        or ci.get("ndcg10")
        or next((v for k, v in ci.items() if k.startswith("ndcg")), {})
    )
    ...
    if fail_rate > warn_rate:
        print(
            f"WARNING: failure_rate {fail_rate:.2%} exceeds warning threshold {warn_rate:.0%}"
        )

    print(f"Winner: {winner.get('variant_id')}")
    print(
        f'  mean_ndcg={mean_ndcg:.4f} CI=[{ndcg_ci.get("lower")}, {ndcg_ci.get("upper")}]'
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--baseline", default="", help="Optional baseline results_summary.json"
    )
