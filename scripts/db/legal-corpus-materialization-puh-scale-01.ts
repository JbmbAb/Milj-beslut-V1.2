...
async function main() {
  ...
  console.log(
    `Slice [${START}, ${START + slice.length}) of ${uniqueObjects.length} unique objects (${manifest.objects.length} total manifest targets)`,
  );
  ...
  for (const obj of slice) {
    ...
    process.stdout.write(
      `\r[${START + idx}/${uniqueObjects.length}] ${obj.file_name}`.slice(0, 100).padEnd(100),
    );
