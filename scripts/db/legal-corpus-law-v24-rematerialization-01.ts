...
async function main() {
  ...
  console.log('projected text hash:', projectedTextHash, `(${extraction.text.length} chars, same projection as Part G)`);
  ...
  if (!v23RowBefore) {
    console.log('!!! Part G v2.3 materialization not found -- this run cannot prove "A unchanged" without it existing first. Aborting.');
  ...
  const v24Admission = admitLawChunksV24({ text: extraction.text, sourceProjectionRef, chunkPolicyVersion: V24_POLICY });

  const v23Dist = chapterDistribution(v23Admission.admitted);
  const v24Dist = chapterDistribution(v24Admission.admitted);
  const v24LetterSuffixed = Object.keys(v24Dist).filter((k) => /[a-z]/i.test(k) && k !== '(ingen kapitelindelning)');

  console.log('\n--- STEP 1: v2.3 vs v2.4 chunk admission comparison (real Miljöbalken text) ---');
  console.log('v2.3: admitted', v23Admission.admitted.length, '| rejected', v23Admission.rejected.length, '| status', v23Admission.document_status);
  console.log('v2.4: admitted', v24Admission.admitted.length, '| rejected', v24Admission.rejected.length, '| status', v24Admission.document_status);
  console.log('v2.3 chapter distribution:', v23Dist);
  console.log('v2.4 chapter distribution:', v24Dist);
  console.log('v2.4 letter-suffixed chapters found:', v24LetterSuffixed.map((k) => `${k} (${v24Dist[k]} chunk[s])`));
  ...
  console.log(
    ...
    'not a real Miljöbalken chapter heading:', crossStatuteDrift.length,
  );
  for (const c of crossStatuteDrift) {
    console.log('  paragraph:', c.structure_kind === 'law' ? c.paragraph : undefined, '| preview:', c.full_text.slice(0, 160));
  }

  const genuineLetterSuffixed = v24Admission.admitted.filter(
    (c) => c.structure_kind === 'law' && c.chapter === '17 a',
  );
  console.log('\ngenuine letter-suffixed chapter correctly captured ("17 a kap.", a real repealed Miljöbalken chapter):', genuineLetterSuffixed.length);
  ...
  const run1 = await materializeOnce(v24Admission.admitted, rawContentHash, projectedTextHash, rawBytes, V24_POLICY, 'run1');
  const v24Row1 = await prisma.legalCorpusMaterialization.findUnique({ where: { canonicalRecordKey: run1.result.canonical_record_key } });
  ...
  console.log('v2.4 record row:', run1.result.corpus_record_id, '| provenance row:', v24Row1?.id, '| governed chunk rows:', v24ChunkCount1);
  console.log('v2.4 materialization_id differs from v2.3 materialization_id:', run1.result.canonical_record_key !== v23DocumentId);

  console.log('\n--- STEP 3: v2.4 materialization run 2 (replay) ---');
  const run2 = await materializeOnce(v24Admission.admitted, rawContentHash, projectedTextHash, rawBytes, V24_POLICY, 'run2');
  const v24ChunkCount2 = v24Row1 ? await countChunks(v24Row1.id) : -1;
  const v24RecordRowCount = await prisma.legalCorpusRecord.count({ where: { recordKey: run1.result.canonical_record_key } });
  ...
  console.log('same chunk row count across runs:', replaySameChunkCount, `(${v24ChunkCount1} vs ${v24ChunkCount2})`);
  console.log('record rows for this key:', v24RecordRowCount, '(no duplicates:', replayNoDuplicateRecords + ')');
  ...
  const v23Untouched = v23RowAfter !== null
  ...
  console.log(JSON.stringify({
    ...
    known_drift: {
      ...
      description: 'unanchored chapterRegex (inherited from v2.3) can pick up a cross-reference to a DIFFERENT statute\'s letter-suffixed chapter embedded mid-paragraph; reported, not fixed, by this unit',
    },
    overall_status:
      v23Untouched && (run1.result.canonical_record_key !== v23DocumentId) && replaySameId && replaySameChunkCount && replayNoDuplicateRecords && replayIdentityStable
        ? 'PROVEN'
        : 'PARTIAL',
  }, null, 2));
