...
async function main() {
  ...
  try {
    const dbs = await p.$queryRawUnsafe<any[]>(`SELECT datname FROM pg_database WHERE datistemplate = false`);
    ...
    if (rasterTables.length === 0) {
      ...
      console.log(
        'Schema'.padEnd(12) + 'Table'.padEnd(45) + 'SRID'.padEnd(8) + 'Bands'.padEnd(8) + 'Resolution',
      );
