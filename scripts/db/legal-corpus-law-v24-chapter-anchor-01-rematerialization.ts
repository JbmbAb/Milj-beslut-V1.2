...
const DOWNLOAD_MANIFEST_REF = {
  ...
  content_hash: { algorithm: 'sha256' as const, digest: '330b1b6031bb712bdab1e2bde35217f42acb3d875a1ef0baa704245643047c1f' },
};
...
function identityFor(chunkPolicyVersion: string, rawContentHash: string, projectedTextHash: string) {
  return {
...
async function materializeOnce(
  ...
  const identity = identityFor(chunkPolicyVersion, rawContentHash, projectedTextHash);
...
async function main() {
  ...
  const extraction = await adapter.extract(
    { ref: { artifact_id: QUARANTINE_ID, artifact_type: 'raw_source' }, doc_name: QUARANTINE_ID, mime_type: MIME_TYPE },
    rawBytes,
  );
  ...
  console.log('projected text hash:', projectedTextHash, `(${extraction.text.length} chars, same projection as all prior Miljöbalken runs)`);

  const { buildCanonicalLegalCorpusRecordKey } = await import('@miljobeslut/mps-legal-corpus');

  // ---------- STEP 0: confirm both prior rows (v2.3 and the pre-fix v2.4) are present, and will
  // remain untouched. ----------
  const v23DocumentId = buildCanonicalLegalCorpusRecordKey(identityFor(V23_POLICY, rawContentHash, projectedTextHash));
  const v24PreFixDocumentId = buildCanonicalLegalCorpusRecordKey(identityFor(V24_PREFIX_POLICY, rawContentHash, projectedTextHash));

  const v23RowBefore = await prisma.legalCorpusMaterialization.findUnique({ where: { canonicalRecordKey: v23DocumentId } });
  const v24PreFixRowBefore = await prisma.legalCorpusMaterialization.findUnique({ where: { canonicalRecordKey: v24PreFixDocumentId } });
  ...
  console.log('pre-fix v2.4 row exists:', v24PreFixRowBefore !== null, '| chunk rows:', v24PreFixChunkCountBefore);
  if (!v23RowBefore || !v24PreFixRowBefore) {
    console.log('!!! one or both prior rows missing -- this run cannot prove "both prior rows unchanged" without them existing first. Aborting.');
  ...
  const v241Admission = admitLawChunksV24({ text: extraction.text, sourceProjectionRef, chunkPolicyVersion: V24_1_POLICY });
  const v241Dist = chapterDistribution(v241Admission.admitted);

  console.log('\n--- STEP 1: v2.4.1 (anchor-fixed) admission ---');
  console.log('admitted:', v241Admission.admitted.length, '| rejected:', v241Admission.rejected.length, '| status:', v241Admission.document_status);
  ...
  const genuineSeventeenA = v241Admission.admitted.filter((c) => c.structure_kind === 'law' && c.chapter === '17 a');
  ...
  console.log('chunks with chapter "17 a" (expected 0 -- chapter is repealed/empty, correctly superseded going forward by "18"):', genuineSeventeenA.length);

  const paragraph19 = v241Admission.admitted.find((c) => c.structure_kind === 'law' && c.paragraph === '19' && c.full_text.includes('sjölagen'));
  console.log('\nparagraph 19 (the fragment containing the sjölagen cross-reference) now labeled chapter:', paragraph19 && paragraph19.structure_kind === 'law' ? paragraph19.chapter : '(not found)');
  ...
  const run1 = await materializeOnce(v241Admission.admitted, rawContentHash, projectedTextHash, rawBytes, V24_1_POLICY, 'run1');
  ...
  console.log('v2.4.1 record row:', run1.result.corpus_record_id, '| provenance row:', v241Row1?.id, '| governed chunk rows:', v241ChunkCount1);
  console.log('distinct from v2.3 id:', run1.result.canonical_record_key !== v23DocumentId, '| distinct from pre-fix v2.4 id:', run1.result.canonical_record_key !== v24PreFixDocumentId);

  console.log('\n--- STEP 3: v2.4.1 materialization run 2 (replay) ---');
  const run2 = await materializeOnce(v241Admission.admitted, rawContentHash, projectedTextHash, rawBytes, V24_1_POLICY, 'run2');
  ...
  console.log('same chunk row count across runs:', replaySameChunkCount, `(${v241ChunkCount1} vs ${v241ChunkCount2})`);
  console.log('record rows for this key:', v241RecordRowCount, '(no duplicates:', replayNoDuplicateRecords + ')');
  ...
  const v24PreFixRowAfter = await prisma.legalCorpusMaterialization.findUnique({ where: { canonicalRecordKey: v24PreFixDocumentId } });
  ...
  const v23Untouched = v23RowAfter !== null && v23RowBefore.id === v23RowAfter.id && v23ChunkCountBefore === v23ChunkCountAfter && v23RowBefore.createdAt.getTime() === v23RowAfter.createdAt.getTime();
  const v24PreFixUntouched = v24PreFixRowAfter !== null && v24PreFixRowBefore.id === v24PreFixRowAfter.id && v24PreFixChunkCountBefore === v24PreFixChunkCountAfter && v24PreFixRowBefore.createdAt.getTime() === v24PreFixRowAfter.createdAt.getTime();

  console.log('\n--- STEP 4: prior materializations after this run ---');
  console.log('v2.3 untouched (same pk/chunk-count/createdAt):', v23Untouched, `(${v23ChunkCountBefore} vs ${v23ChunkCountAfter})`);
  console.log('pre-fix v2.4 untouched (same pk/chunk-count/createdAt):', v24PreFixUntouched, `(${v24PreFixChunkCountBefore} vs ${v24PreFixChunkCountAfter})`);

  console.log('\n\n========== LEGAL-CHUNKING-LAW-V2.4-CHAPTER-ANCHOR-01 SUMMARY ==========');
  console.log(JSON.stringify({
    source: SOURCE_ID,
    quarantine_id: QUARANTINE_ID,
    prior_v23: { materialization_id: v23DocumentId, untouched: v23Untouched, chunk_rows: v23ChunkCountAfter },
    prior_v24_prefix: { materialization_id: v24PreFixDocumentId, untouched: v24PreFixUntouched, chunk_rows: v24PreFixChunkCountAfter, note: 'left as-is, honest historical record of the bug' },
    v24_1: {
    ...
    overall_status:
      v23Untouched && v24PreFixUntouched && falseTenA.length === 0
        && (run1.result.canonical_record_key !== v23DocumentId) && (run1.result.canonical_record_key !== v24PreFixDocumentId)
        && replaySameId && replaySameChunkCount && replayNoDuplicateRecords && replayIdentityStable
        ? 'PROVEN'
        : 'PARTIAL',
  }, null, 2));
