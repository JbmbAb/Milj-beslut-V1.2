...
async function run() {
    ...
    const searchUrl = `${baseUrl}/administrativ-indelning/collections/kommuner/items?f=json&limit=1`;
    ...
    try {
        ...
        if (data.features?.length) {
            console.log("First commune: " + data.features[0].properties.kommunnamn);
        } else {
