...
function record(overrides = {}) {
  const value = {
    schema_version: 'dev-gov-v0-trusted-execution-record',
...
function signedPair(privateKey) {
  ...
  const green = signExecutionRecord(
    record({
      execution_sha: manifest.target_sha,
      proof_type: 'GREEN',
      test_id: 'green',
...
describe('DEV-GOV-V0 trusted execution authority', () => {
  it('accepts exact RED/GREEN claims signed by the externally trusted issuer', () => {
    ...
    const result = evaluateTrustedExecutionGate(manifest, signedPair(privateKey), policy(publicKey));
  ...
  it('denies correctly signed execution for the wrong SHA or command', () => {
    ...
    const result = evaluateTrustedExecutionGate(manifest, [red, wrongGreen], policy(trusted.publicKey));
  ...
  it('denies attestations from an unexpected protected workflow or runner identity', () => {
    ...
    const result = evaluateTrustedExecutionGate(manifest, attestations, wrongPolicy);
  ...
  it('denies missing trusted execution without downgrading it to local evidence', () => {
    ...
    const result = evaluateTrustedExecutionGate(manifest, [], policy(trusted.publicKey));
