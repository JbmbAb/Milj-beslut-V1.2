...
async function runBulkImport(filePath: string, targetKey: keyof typeof TARGETS) {
  ...
  try {
    ...
    const ogrCmd = `ogr2ogr -f PostgreSQL "${pgConn}" "${filePath}" ` +
    ...
    await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS ${config.table.replace('.', '_')}_${config.geomCol}_idx ON ${config.table} USING GIST (${config.geomCol});`);
