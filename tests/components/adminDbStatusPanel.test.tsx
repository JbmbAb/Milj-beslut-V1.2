...
describe('AdminDbStatusPanel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // localStorage.getItem returnerar null som standard → DEMO_TOKEN används
  ...
  it('visar statistiksiffror och grön badge när hämtning lyckas', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('db-kpi-documents-value')).toHaveTextContent('42'),
    );
  ...
  it('visar felmeddelande om API returnerar fel', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Databas ej tillgänglig/i)).toBeInTheDocument(),
    );
  ...
  it('"Uppdatera"-knapp triggar ny hämtning', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('db-kpi-documents-value')).toHaveTextContent('99'),
    );
