...
vi.mock('@miljobeslut/mps-runtime', async (importOriginal) => {
  ...
  return {
    ...
    MimersIntegration: {
      create: async () => ({
        artifactRepository: mimersRepoBox.repo,
        rebuildIndex: async () => ({ rebuilt: 0, skipped: 0 }),
      }),
    },
  };
});
...
vi.mock('../../server/modules/localization/localizationGeometrySupersessionQueue', () => {
  ...
  type Rec = {
    ...
    status: Status;
    supersessionArtifactId: string | null;
  ...
  let counter = 0;
  return {
    enqueueLocalizationGeometrySupersessionRequest: vi.fn(
      async (input: {
        projectId: string;
        predecessorGeometryArtifactId: string;
        successorGeometryArtifactId: string;
        ...
        const rec: Rec = {
          id: `fake-supersession-req-${++counter}`,
    ...
    ensureLocalizationGeometrySupersessionRequested: vi.fn(
      async (input: {
        projectId: string;
        predecessorGeometryArtifactId: string;
        successorGeometryArtifactId: string;
        ...
        const existing = rows.find(
          (r) =>
            ...
            r.predecessorGeometryArtifactId === input.predecessorGeometryArtifactId &&
            r.successorGeometryArtifactId === input.successorGeometryArtifactId,
        );
    ...
    leaseOnePendingLocalizationGeometrySupersessionRequest: vi.fn(async () => {
      ...
      candidate.leaseToken = `lease-token-${candidate.id}`;
      return candidate;
    }),
    markLocalizationGeometrySupersessionCompleted: vi.fn(
      async (id: string, leaseToken: string, supersessionArtifactId: string) => {
        const row = rows.find((r) => r.id === id && r.status === 'LEASED' && r.leaseToken === leaseToken);
        if (row) {
          row.status = 'COMPLETED';
          row.supersessionArtifactId = supersessionArtifactId;
    ...
    markLocalizationGeometrySupersessionFailed: vi.fn(
      async (id: string, leaseToken: string, failureCode: string, failureDetail: string) => {
        const row = rows.find((r) => r.id === id && r.status === 'LEASED' && r.leaseToken === leaseToken);
        if (row) {
          row.status = 'FAILED';
    ...
    markLocalizationGeometrySupersessionSuperseded: vi.fn(
      async (id: string, leaseToken: string, detail: string) => {
        const row = rows.find((r) => r.id === id && r.status === 'LEASED' && r.leaseToken === leaseToken);
        if (row) {
          row.status = 'SUPERSEDED';
          row.failureCode = 'PREDECESSOR_NO_LONGER_CURRENT';
    ...
    getSupersessionRequestStatusForSubject: vi.fn(
      async (projectId: string, predecessorGeometryArtifactId: string, successorGeometryArtifactId: string) =>
        rows.find(
          (r) =>
            r.projectId === projectId &&
            r.predecessorGeometryArtifactId === predecessorGeometryArtifactId &&
            r.successorGeometryArtifactId === successorGeometryArtifactId,
        ) ?? null,
    ),
    getLatestSupersessionRequestForProject: vi.fn(
      async (projectId: string) => [...rows].reverse().find((r) => r.projectId === projectId) ?? null,
    ),
  };
});
...
const geometrySupersessionIssuerKey = LocalPemSigningKeyProvider.generate(
  'ed25519:geometry-supersession-issuer-drawing-proofs',
);
process.env.LOCALIZATION_GEOMETRY_SUPERSESSION_ISSUER_KEY_ID = geometrySupersessionIssuerKey.provider.keyId;
process.env.LOCALIZATION_GEOMETRY_SUPERSESSION_ISSUER_PRIVATE_KEY_PEM =
  geometrySupersessionIssuerKey.privateKey;
...
describe('PRODUCT-LU-CESIUM-LOCALIZATION-DRAWING-01 — save/read proof matrix', () => {
  ...
  it('proof: unauthorized user -> DENY', async () => {
    ...
    const issuer = createProjectContextBindingIssuerArtifact({
  ...
  it('proof: GET before any save -> transitional derived_from_property_boundary state', async () => {
    ...
    if (result.ok === true) expect(result.data.provenance).toBe('derived_from_property_boundary');
  });
  ...
  it('proof: coordinate contract -- WGS84 input -> deterministic transform -> persisted SWEREF -> read-back same location', async () => {
    ...
    const [readLat, readLng] = fakeSweref99ToWgs84(
      stored.payload.coordinates[0],
      stored.payload.coordinates[1],
    );
  ...
  it('proof: same exact point saved again -> idempotent identity, no divergent state', async () => {
    ...
    const current = await getCurrentLocalizationGeometryForProject({
      authUser: owner,
      projectId,
      artifactRepository: repo,
      spatialRuntime,
  ...
  it('proof: move point A -> B -- new artifact, B current, A historical, refresh reloads B', async () => {
    ...
    const b = await saveUserLocalizationGeometry({
      authUser: owner,
      projectId,
      input: { geometry_type: 'POINT', coordinates: [18.2, 59.4], srid: 4326 },
      artifactRepository: repo,
      spatialRuntime,
    });
    ...
    const stillA = await getCurrentLocalizationGeometryForProject({
      authUser: owner,
      projectId,
      artifactRepository: repo,
      spatialRuntime,
    ...
    const reloaded = await getCurrentLocalizationGeometryForProject({
      authUser: owner,
      projectId,
      artifactRepository: repo,
      spatialRuntime,
    ...
    const historicalA = await repo.resolve<{ artifact_id: string }>({
      artifact_id: a.data.artifact_id,
      artifact_type: 'localization_geometry',
    });
