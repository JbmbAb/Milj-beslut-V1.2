...
describe('PUBLIC-PRISMA-BASELINE-RECONCILIATION-01 capture tooling', () => {
  it('keeps the catalog probe SELECT-only', () => {
    ...
    expect(executableSql).toContain('con.condeferrable AS is_deferrable');
    expect(executableSql).toContain("'deferrable', is_deferrable");
    expect(executableSql).not.toMatch(/\bAS\s+deferrable\b/i);
  });
