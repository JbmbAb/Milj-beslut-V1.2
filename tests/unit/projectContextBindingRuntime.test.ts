...
describe("P3-LU-PROJECT-CONTEXT-BINDING-RUNTIME-01", () => {
  ...
  it("resolves a verified supersession successor as the only current head after fresh construction", async () => {
    ...
    const relation = createProjectContextBindingSupersessionArtifact({
      ...
      reason_code: "CANONICAL_PROPERTY_COORDINATE_ORDER_CORRECTION", issuer_ref: v2Authority,
      issuer_key_id: issuerKey.provider.keyId, issued_at: "2026-08-21T13:00:00.000Z",
    });
    const signedRelation = { ...relation, attestation: await attestProjectContextBindingSupersessionArtifact({ artifact: relation, issuer: v2Issuer, signing: issuerKey.provider }) };
  ...
  it("rejects supersession authority from a V1 issuer even with a valid signing key", async () => {
    const oldBinding = createProjectContextBindingArtifact({
      project_id: "project-a", project_context_ref: contextA, project_property_binding_ref: propertyBinding,
    ...
    await expect(attestProjectContextBindingSupersessionArtifact({ artifact: relation, issuer, signing: issuerKey.provider }))
  ...
  it("freshly reopens persistent CAS and derives the same verified current head from a rebuilt projection", async () => {
    ...
    try {
      ...
      const unsigned = createProjectContextBindingSupersessionArtifact({ contract_version: "PROJECT_CONTEXT_BINDING_SUPERSESSION_V1", project_id: "project-a", superseded_binding_ref: { artifact_id: signedOld.artifact_id, artifact_type: signedOld.artifact_type }, successor_binding_ref: { artifact_id: signedSuccessor.artifact_id, artifact_type: signedSuccessor.artifact_type }, reason_code: "CORRECTION", issuer_ref: v2Authority, issuer_key_id: issuerKey.provider.keyId, issued_at: "2026-08-21T01:00:00.000Z" });
      const signed = { ...unsigned, attestation: await attestProjectContextBindingSupersessionArtifact({ artifact: unsigned, issuer: v2Issuer, signing: issuerKey.provider }) };
