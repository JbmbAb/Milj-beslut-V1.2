...
import { HTML_EXTRACT_ADAPTER_VERSION } from '../../server/text-projection/versions';
...
describe('PdfParseExtractorAdapter — governed HTML projection', () => {
  ...
  it('never mutates the raw input bytes', async () => {
    ...
    // Array.from(), not a direct Uint8Array toEqual: under this project's jsdom environment,
    // two byte-identical Uint8Array instances from separate TextEncoder calls can belong to
    // different realms and compare unequal by chai's deep-equal even with identical contents --
    // confirmed by comparing under plain Node, where the same bytes ARE toEqual. Converting to a
    // plain array sidesteps that environment quirk without weakening what this actually checks.
