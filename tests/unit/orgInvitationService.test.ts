...
describe('acceptInvitation', () => {
  ...
  it('accepts a valid invitation and returns user info', async () => {
    ...
    mocks.userCreate.mockResolvedValue({ id: 'new-user-001', bankidId: 'bid123', organisationId: ORG_ID, role: 'CONSULTANT' });
  ...
  it('reuses existing user if found', async () => {
    ...
    mocks.userFindFirst.mockResolvedValue({ id: 'existing-user', bankidId: 'bid-existing', organisationId: ORG_ID, role: 'CONSULTANT' });
  ...
  it('throws when invitation is already accepted', async () => {
    ...
    mocks.userFindFirst.mockResolvedValue(null);
    mocks.userCreate.mockResolvedValue({ id: 'u1', bankidId: 'b1', organisationId: ORG_ID, role: 'CONSULTANT' });
    ...
    // Try again
    await expect(
      acceptInvitation({ orgId: ORG_ID, token: inv.token, bankidId: 'b1' }),
