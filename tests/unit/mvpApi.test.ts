...
describe('mvpApi.express', () => {
  ...
  it('lists MVP projects for the authenticated member with coverage percentages', async () => {
    mocks.projectMemberFindMany.mockResolvedValue([
      {
        project: {
          id: 'project-1',
  ...
  it('searches inside an MVP project after membership verification', async () => {
    vi.mocked(runSearchQuery).mockResolvedValue({
      results: [
        {
          documentId: 'doc-1',
  ...
  it('returns 403 for MVP project search when membership verification fails', async () => {
