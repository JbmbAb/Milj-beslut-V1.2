...
describe('extractAndGenerateChecklistFromRag', () => {
  it('returns created counts on a successful run', async () => {
    const result = await extractAndGenerateChecklistFromRag(
      'proj-1',
      'org-1',
      'lakvatten krav',
      '90.003-1'
  ...
  it('throws when embedding generation fails', async () => {
    ...
    await expect(
      extractAndGenerateChecklistFromRag('proj-1', 'org-1', 'query', 'code')
  ...
  it('throws when AI returns invalid JSON', async () => {
    ...
    await expect(
      extractAndGenerateChecklistFromRag('proj-1', 'org-1', 'query', 'code')
    ).rejects.toThrow('AI failed to return valid JSON format');
  ...
  it('handles multiple requirements across the same document', async () => {
    mocks.serverGenerateText.mockResolvedValue(
      JSON.stringify([
        { documentId: 'doc-1', category: 'Cat1', requirementTextQuote: 'Q1' },
        { documentId: 'doc-1', category: 'Cat2', requirementTextQuote: 'Q2' },
      ])
    );
