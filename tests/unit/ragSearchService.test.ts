...
describe('runRagSearch', () => {
  beforeEach(() => {
    ...
    // Re-apply constructor implementation after resetAllMocks clears it.
    genAiMock.GoogleGenAI.mockImplementation(function () {
  ...
  it('returns answer with sources and graph nodes on happy path', async () => {
    ...
    mocks.generateContent.mockResolvedValue({ text: 'Svaret är att miljöbalken kräver tillstånd.' });
  ...
  it('warns and continues when graph search throws', async () => {
    ...
    expect(mocks.loggerWarn).toHaveBeenCalledWith(
      'rag-search: graph search failed',
      expect.anything(),
    );
  });

  it('skips Gemini when no API key is set', async () => {
