...
const mocks = vi.hoisted(() => {
  ...
  function PrismaClientMock(this: unknown) { return prismaInstance; }
...
describe('segmentText', () => {
  ...
  it('handles page breaks (\\f) and sets pageNumber', () => {
    const text = 'Sida ett innehåller krav på säkerhet.\fSida två innehåller krav på avfall och rapportering.';
    const segments = segmentText(text);
...
describe('processPendingAttachments', () => {
  ...
  it('processes attachment with valid .txt storedPath', async () => {
    ...
    mockReadFile.mockResolvedValue(
      'Dagvatten ska hanteras via oljeavskiljare och uppsamlingstank på anläggningen.'
    );
...
describe('queryRequirements', () => {
  ...
  it('applies municipality filter when provided', async () => {
    ...
    const call = mocks.extractedRequirementFindMany.mock.calls[0][0];
    expect(call.where.municipality).toBe('Malmö');
  });
});
...
describe('getRequirementStats', () => {
  ...
  it('returns total, byCategory, and byMunicipality', async () => {
    mocks.extractedRequirementGroupBy
      .mockResolvedValueOnce([{ category: 'sampling', _count: { id: 4 } }])
      .mockResolvedValueOnce([{ municipality: 'Stockholm', _count: { id: 2 } }]);
    ...
    expect(stats.byMunicipality).toHaveLength(1);
  });
