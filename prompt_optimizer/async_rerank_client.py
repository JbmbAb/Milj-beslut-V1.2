...
class AsyncRerankClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_retries = max_retries if max_retries is not None else int(os.environ.get('RERANK_MAX_RETRIES', '4'))
        self.persistent_cache = persistent_cache or PersistentCache()
        self.input_usd = input_usd or float(os.environ.get('VERTEX_INPUT_USD_PER_TOKEN', DEFAULT_INPUT_USD_PER_TOKEN))
        self.output_usd = output_usd or float(os.environ.get('VERTEX_OUTPUT_USD_PER_TOKEN', DEFAULT_OUTPUT_USD_PER_TOKEN))
    ...
    async def rerank(
        ...
        *,
        ...
        if cached is not None:
            lat = LatencyBreakdown(**{k: v for k, v in cached['latency'].items() if k.endswith('_ms')})
        ...
        async for attempt in AsyncRetrying(
            ...
            with attempt:
                ...
                resp = await client.post(self.http_url, content=body, headers={'Content-Type': 'application/json'})
                ...
                out_tok = int(data.get('output_tokens', _estimate_tokens(json.dumps(items))))
