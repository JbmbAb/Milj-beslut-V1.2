...
class Config(BaseSettings):
    ...
    retry_backoff_multiplier: float = Field(default=0.5, alias='RERANK_BACKOFF_MULTIPLIER', gt=0)
    ...
    # Failure budgets
    warning_failure_rate: float = Field(default=0.02, alias='WARNING_FAILURE_RATE', ge=0, le=1)
    hard_failure_rate: float = Field(default=0.05, alias='HARD_FAILURE_RATE', ge=0, le=1)

    # Paths and outputs
    cache_dir: str = Field(default='.', alias='CACHE_DIR')
    cache_path: str = Field(default='./rerank_eval_cache.sqlite', alias='RERANK_CACHE_PATH')
    ...
    latency_budget_p95_s: float | None = Field(default=None, alias='LATENCY_BUDGET_P95_S')
    max_cost_per_query_usd: float | None = Field(default=None, alias='MAX_COST_PER_QUERY_USD')
    ...
    @field_validator('max_est_cost_usd', 'latency_budget_p95_s', 'max_cost_per_query_usd', mode='before')
    @classmethod
