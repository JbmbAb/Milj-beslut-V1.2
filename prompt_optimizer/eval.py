...
def _lexical_fallback_rank(query: str, candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for c in candidates:
        ...
        scored.append({'id': c['id'], 'score': float(c.get('score', 0.5)) + matches * 0.1})
...
def _aggregate_metric_lists(per_query: list[dict[str, Any]], prefix: str) -> dict[str, float]:
    ...
    def eval_one(record: dict[str, Any]) -> dict[str, Any]:
        ...
        with semaphore:
            ...
            try:
                ...
                limiter.record_tokens(resp.input_tokens + resp.output_tokens, est_tokens)
