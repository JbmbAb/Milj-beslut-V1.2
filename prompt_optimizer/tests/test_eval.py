...
class CacheTest(unittest.TestCase):
    def test_cache_key_includes_schema_version(self) -> None:
        key = build_cache_key(
            prompt_hash="a", query_id="q", candidate_hash="c", reranker_version="v"
        )
...
class EvalTest(unittest.TestCase):
    ...
    def test_score_with_mock_and_resume(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            template = 'Rank "{{QUERY}}".\n\n{{DOCUMENTS}}'
            r1 = score_prompt_variant(
                records, template, variant_id="v1", client=client, max_workers=1
            )
            r2 = score_prompt_variant(
                records, template, variant_id="v1", client=client, max_workers=1
            )
