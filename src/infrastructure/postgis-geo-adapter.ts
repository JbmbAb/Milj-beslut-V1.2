...
import type { Bbox } from '../../server/utils/geo/bbox';
import { findDatasetMapLayer } from '../../server/datasources/platformMapLayerRegistry';
...
export class PostgisGeoAdapter {
  ...
  quotePgIdentifier(identifier: string): string {
  ...
  safeParseGeometry(geojson: string): GeoJSON.Geometry | null {
    try {
  ...
  async getDatasetMapLayer(
    ...
    if (!(await this.tableExists(def.schema, def.table))) {
      return {
        type: 'FeatureCollection',
        features: [],
        meta: {
    ...
    const schemaSql = this.quotePgIdentifier(def.schema);
    const tableSql = this.quotePgIdentifier(def.table);
    ...
    const features = rows
      .map((row) => {
        const geometry = this.safeParseGeometry(row.geojson);
