/**
 * Evaluate Sewage Regulations Use Case
...
import { listSewageEvidenceSources } from '../../server/modules/legal/catalogs/sewageEvidenceSources';
...
export const SEWAGE_REGULATIONS: SewageRegulation[] = [
    ...
    references: [
      {
        ...
        sourceTracing: {
          source: 'LOCAL_RULES',
          timestamp: '2026-07-04T15:58:49Z',
          version: '1998:808',
        },
      },
    ],
...

  {
    ...
    requirement:
      'I högt skyddade områden (vattenskyddsområde, Natura 2000) krävs minireningsverk med kemfällning (BDTA) eller better',
...
export function validateSewageApplicationRegulations(
  ...
  if (
    ['INFILTRATION', 'SOIL_BED'].includes(systemType) &&
    protectionProfile.nearestWell.distance < 50 &&
    protectionProfile.nearestWell.owner === 'OWN'
  ) {
    violations.push(
  ...
  if (
    ['INFILTRATION', 'SOIL_BED', 'MINI_PLANT_BDT', 'MINI_PLANT_BDTA'].includes(systemType) &&
    protectionProfile.nearestWell.owner === 'NEIGHBOR' &&
    protectionProfile.nearestWell.distance < 50
  ) {
    if (!application.neighborConsentObtained) {
      violations.push(
        `Grannens brunn ligger endast ${protectionProfile.nearestWell.distance}m bort. Grannemedgivande är OBLIGATORISKT för brunnar inom 50m.`,
      );
    }
  }
...
export interface EvaluateSewageRegulationsInput {
  systemType: SewageSystemTypeId;
  protectionLevel: 'NORMAL' | 'HIGH';
  municipalityCode: string;
  distanceData?: {
...
export interface ValidateSewageApplicationInput {
  application: SewageApplication;
  protectionProfile: SewageProtectionProfile;
}

/**
 * UseCase Class for Sewage Regulations Evaluation
 */
export class EvaluateSewageRegulationsUseCase {
  async execute(input: EvaluateSewageRegulationsInput): Promise<SewageRequirement[]> {
    return generateSewageRequirementChecklist(
      input.systemType,
  ...
  async validate(input: ValidateSewageApplicationInput) {
    return validateSewageApplicationRegulations(input.application, input.protectionProfile);
  }
}
